...
def validate_feature_matrix_contract(df: pd.DataFrame, expected_columns: list) -> list:
    ...
    Confirms a feature matrix (from build_features.build_feature_matrix)
    matches the exact column set a loaded model expects, BEFORE calling
    model.predict() — a mismatch here previously surfaced as an opaque
    sklearn shape-mismatch exception deep inside inference.
    """
    issues = []
    ...
    if missing:
        issues.append(f"Feature matrix is missing columns the model expects: {sorted(missing)}")
    if extra:
        issues.append(f"Feature matrix has unexpected extra columns: {sorted(extra)}")
    if df.isna().any().any():
        issues.append("Feature matrix contains NaN values.")
    return issues
